...
if [[ -n "${_YAD_UI_SH_LOADED:-}" ]]; then
    return 0
fi
_YAD_UI_SH_LOADED=1

# Base GTK CSS with Catppuccin Mocha Color Palette Definitions for YAD dialogs
BASE_CSS="
...
show_error() {
    ...
    while getopts "s:v:" opt; do
        case "$opt" in
            s) message="$OPTARG" ;;
            v) vars="$OPTARG" ;;
            *) echo "Usage: show_error [-s message] [-v vars]" >&2; exit 1 ;;
        esac
    done
    ...
    # Re-exec calling script with preserved form variables
    exec "$0" -v "$vars"
}
